...
def load_settings_from_file():
    """Kayıt dosyasından (settings.json) ayarları yükler."""
    global WIDTH, HEIGHT, TARGET_FPS, PADDLE_HEIGHT, BALL_SIZE, bg_hsv, paddle_hsv, ball_hsv
    if os.path.exists(SETTINGS_FILE):
        try:
            with open(SETTINGS_FILE, "r") as f:
                ...
                TARGET_FPS = data.get("target_fps", DEFAULT_FPS)
                PADDLE_HEIGHT = data.get("paddle_height", DEFAULT_PADDLE_HEIGHT)
                BALL_SIZE = data.get("ball_size", DEFAULT_BALL_SIZE)
                bg_hsv = data.get("bg_hsv", [0.0, 0.0, 0.0])
                paddle_hsv = data.get("paddle_hsv", [0.0, 0.0, 1.0])
                ball_hsv = data.get("ball_hsv", [0.0, 0.0, 1.0])
        except Exception as e:
            print(f"Kayıt dosyası okunurken hata oluştu: {e}")

def save_settings_to_file():
    """Mevcut ayarları settings.json dosyasına kaydeder."""
    data = {
        "width": WIDTH,
        "height": HEIGHT,
    ...
    try:
        ...
        print(f"Kayıt dosyası yazılırken hata oluştu: {e}")

# Oyunu açarken kaydedilmiş ayarları yükle
...
# 5 Metin Kutusu (Width, Height, FPS, Paddle, Ball) + 9 Slider + 3 Buton = 17 Öğe
SETTINGS_ITEMS_COUNT = 17
...
def apply_all_settings():
    global WIDTH, HEIGHT, TARGET_FPS, PADDLE_HEIGHT, BALL_SIZE, screen, p1_y, p2_y, ball_x, ball_y
    try:
        w = int(width_text)
        h = int(height_text)
        if w >= 400 and h >= 300:
    ...
    try:
        ...
        TARGET_FPS = max(15, min(240, fps_val))
    except ValueError:
        pass
...
def reset_defaults():
    global WIDTH, HEIGHT, TARGET_FPS, PADDLE_HEIGHT, BALL_SIZE, bg_hsv, paddle_hsv, ball_hsv
    global width_text, height_text, fps_text, paddle_len_text, ball_size_text, screen
    WIDTH = DEFAULT_WIDTH
    HEIGHT = DEFAULT_HEIGHT
    TARGET_FPS = DEFAULT_FPS
    PADDLE_HEIGHT = DEFAULT_PADDLE_HEIGHT
...
while True:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 4:
                    ...
                    text_boxes = [
                        ...
                        (4, WIDTH // 2 + 20, start_y + 180, 160, 35)
                    ]
                    ...
                    sliders = [
                        (5, bg_hsv, 0, start_y + 225),
                        (6, bg_hsv, 1, start_y + 270),
                        (7, bg_hsv, 2, start_y + 315),
                        (8, paddle_hsv, 0, start_y + 360),
                        (9, paddle_hsv, 1, start_y + 405),
                        (10, paddle_hsv, 2, start_y + 450),
                        (11, ball_hsv, 0, start_y + 495),
                        (12, ball_hsv, 1, start_y + 540),
                        (13, ball_hsv, 2, start_y + 585)
                    ]
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    active_text_box = settings_focus if settings_focus in range(0, 5) else None
                    adjust_scroll_to_focus()
                elif event.key == pygame.K_DOWN or event.key == pygame.K_s:
                    settings_focus = (settings_focus + 1) % SETTINGS_ITEMS_COUNT
                    active_text_box = settings_focus if settings_focus in range(0, 5) else None
                    adjust_scroll_to_focus()
                elif event.key == pygame.K_LEFT and settings_focus in range(5, 14):
                    sliders_map = [
                        (bg_hsv, 0), (bg_hsv, 1), (bg_hsv, 2),
                        (paddle_hsv, 0), (paddle_hsv, 1), (paddle_hsv, 2),
                        (ball_hsv, 0), (ball_hsv, 1), (ball_hsv, 2)
                    ]
                    target, h_i = sliders_map[settings_focus - 5]
                    target[h_i] = max(0.0, target[h_i] - 0.05)
                elif event.key == pygame.K_RIGHT and settings_focus in range(5, 14):
                    sliders_map = [
                        (bg_hsv, 0), (bg_hsv, 1), (bg_hsv, 2),
                        (paddle_hsv, 0), (paddle_hsv, 1), (paddle_hsv, 2),
                        (ball_hsv, 0), (ball_hsv, 1), (ball_hsv, 2)
                    ...
                    if settings_focus == 14 or active_text_box is not None:
                        apply_all_settings()
                        state = "MENU"
                    elif settings_focus == 15:
                        reset_defaults()
                    elif settings_focus == 16:
                        state = "MENU"
                    ...
                    if event.unicode.isdigit() and active_text_box is not None:
                        ...
                        elif active_text_box == 2 and len(fps_text) < 3:
                            fps_text += event.unicode
                        elif active_text_box == 3 and len(paddle_len_text) < 3:
                            paddle_len_text += event.unicode
                        elif active_text_box == 4 and len(ball_size_text) < 3:
    ...
    if state == "MENU":
        ...
        text_items = [
            ...
            ("Target FPS:", fps_text, 2),
            ("Paddle Length:", paddle_len_text, 3),
            ("Ball Size:", ball_size_text, 4)
        ]
        ...
        slider_items = [
            ("BG Hue:", bg_hsv, 0, 5),
            ("BG Saturation:", bg_hsv, 1, 6),
            ("BG Value:", bg_hsv, 2, 7),
            ("Paddle Hue:", paddle_hsv, 0, 8),
            ("Paddle Saturation:", paddle_hsv, 1, 9),
            ("Paddle Value:", paddle_hsv, 2, 10),
            ("Ball Hue:", ball_hsv, 0, 11),
            ("Ball Saturation:", ball_hsv, 1, 12),
            ("Ball Value:", ball_hsv, 2, 13)
        ]
        ...
        btn_save_y = start_y + 645
        btn_reset_y = start_y + 690
        btn_cancel_y = start_y + 735

        if -40 < btn_save_y < HEIGHT:
            color = GREEN if settings_focus == 14 else GRAY
            btn1 = font.render("[ Save & Apply ]", True, color)
            screen.blit(btn1, (WIDTH // 2 - btn1.get_width() // 2, btn_save_y))

        if -40 < btn_reset_y < HEIGHT:
            color = RED if settings_focus == 15 else GRAY
        ...
        if -40 < btn_cancel_y < HEIGHT:
            color = WHITE if settings_focus == 16 else GRAY
